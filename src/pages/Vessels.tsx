...
const getVesselType = (cls?: string) => {
  switch (cls) {
    case "Cargo":
    case "Cargo Vessel":
      return "Cargo Vessel";
...
export default function Vessels() {
  ...
  const rows = (vQ.data ?? []).filter((v) =>
    v.vessel_id.toLowerCase().includes(q.toLowerCase())
  );
    ...
    <Panel
      ...
      right={
        <div className="relative">
          <Search
            size={12}
            className="absolute left-2 top-1/2 -translate-y-1/2 text-fg-2"
          />
          <input
            value={q}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-2xs uppercase tracking-[0.14em] text-fg-2">
            ...
            <th className="px-4 py-2">Vessel Type</th>
          ...
          {rows.map((v) => (
            ...
            >
              ...
              <td className="px-4 py-2 text-fg-1">
                {getVesselType(v.vessel_class)}
              </td>

              <td className="px-4 py-2">
                <span className={clsx("chip", statusCls(v.status))}>
                  {v.status}
                </span>
              </td>
